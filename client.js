...
if (!window.Store) {
    (function () {
        ...
        if (Object.keys(modules)) {
            ...
            window.Store.GroupMetadata = window.require(
...
const getNotes = () => {
    return new Promise((resolve) => {
        ...
        request.onerror = (event) => {
            console.error(
                "WAXP: Error opening IndexedDB 'model-storage'",
                event,
            );
            resolve({});
        };
        ...
        request.onupgradeneeded = () => {
            // This can happen if the user clears site data. DB needs to be created.
            // In this case, there are no notes. just let it proceed.
            // it will be created by whatsapp web.
            console.log("WAXP: onupgradeneeded for 'model-storage'");
            resolve({});
        };
        ...
        request.onsuccess = (event) => {
            ...
            if (!db.objectStoreNames.contains('note')) {
                console.log("WAXP: 'note' object store not found");
                resolve({});
                return;
            }
            ...
            getAllRequest.onsuccess = (event) => {
                ...
                for (const note of notes) {
                    if (note.chatJid && note.content) {
                        const phone = note.chatJid.split('@')[0];
                        if (notesMap[phone]) {
                            notesMap[phone] += `\n---\n${note.content}`;
                        } else {
                            notesMap[phone] = note.content;
                        }
                    }
                }
                resolve(notesMap);
            ...
            getAllRequest.onerror = (event) => {
                console.error(
                    "WAXP: Error getting notes from 'note' object store",
                    event,
                );
                resolve({});
            };
...
const getLabelAssociationsFromDB = (labelId) => {
    return new Promise((resolve, reject) => {
        const request = indexedDB.open('model-storage');

        request.onerror = (event) => {
            console.error(
                "WAXP: Error opening IndexedDB 'model-storage' for labels",
                event,
            );
            reject(event);
        };
        ...
        request.onupgradeneeded = (event) => {
            console.log(
                "WAXP: onupgradeneeded for 'model-storage' for labels, DB will be created.",
            );
            // DB will be created by WhatsApp, we don't need to do anything.
            // Resolve with empty array as there will be no associations.
            resolve([]);
        };

        request.onsuccess = (event) => {
            const db = event.target.result;
            if (!db.objectStoreNames.contains('label-association')) {
                console.warn(
                    "WAXP: 'label-association' object store not found.",
                );
                resolve([]);
                return;
            }
            ...
            const objectStore = transaction.objectStore('label-association');
            const index = objectStore.index('labelId');
            const getAllRequest = index.getAll(labelId);

            getAllRequest.onsuccess = (event) => {
                const associations = event.target.result;
                const associationIds = associations.map(
                    (assoc) => assoc.associationId,
                );
                resolve(associationIds);
            };

            getAllRequest.onerror = (event) => {
                console.error(
                    `WAXP: Error getting associations for labelId ${labelId}`,
                    event,
                );
                reject(event);
            };
...
const prepareDataForDownload = (data) => {
    ...
    for (let [key, _data] of Object.entries(contactData)) {
        ...
        for (const [i, contact] of _data.contacts.entries()) {
            ...
            if (notes) {
                contact.notes = notes[phone] || '';
            } else {
                contact.notes = ''; // Ensure notes field is always present for business accounts
            }
...
function exportContactsFromSelectedLabel(label, allOrUnsaved) {
    ...
    try {
        ...
        var chatLid = new RegExp(/(?<phone_c>[\d]+)@lid/);
        var finalRe = new RegExp(
            groups.source + '|' + chat.source + '|' + chatLid.source,
        );
        var contacts = Array.from(
            new Set(
                labelItem.labelItemCollection._models.map((m) => {
                    ...
                    if (id.includes('@lid')) {
                        // Handle LID IDs separately
                        const contact = window.Store.Contact.get(id);
                        if (contact && contact.phoneNumber) {
                            return contact.phoneNumber.user;
                        } else {
                            console.log(`Cannot extract LID number from ${id}`);
                            return null;
                        }
                    }
...
const getContactName = (contact) => {
    console.log('Getting name for contact', contact);
    ...
    try {
        ...
        const groups = window.Store.Contact.getGroupContacts()
            ...
            .map((c) => ({
                ...
                count: window.Store.GroupMetadata.get(c.id._serialized)
                    .participants.length,
            }));
 ...
 * @param {Array} filters
 */
const exportChatlistContacts = (filters, notes, isBusinessAccount) => {
    let contacts = getAllChats().filter((chat) =>
        ['c.us', 'lid'].includes(chat.id.server),
    );

    // Filter out enterprise chats
    // contacts = contacts.filter((chat) => !chat.contact.isEnterprise);

    const filterOnlyUnsaved = filters.find(
        (f) => f.id === 'exportOnlyUnsaved',
    )?.checked;
    if (filterOnlyUnsaved) {
        contacts = contacts.filter((chat) => !isMyContact(chat.contact));
    }
    contacts = contacts.map((e) => {
        ...
        } else if (e.id.server === 'lid') {
            phoneNumber =
                e?.contact?.phoneNumber?.user || 'HIDDEN_BY_PRIVACY_SETTINGS';
        }
        return {
...
const exportGroupContacts = (
    ...
    for (const selectedGroup of groupSelection) {
        ...
        contacts = contacts.map((participant) => {
            ...
            let phoneNumber;
            if (participant?.contact?.phoneNumber) {
                phoneNumber = participant?.contact?.phoneNumber?.user;
            } else if (participant.id.server === 'lid') {
                phoneNumber =
...
const exportLabelContacts = async (labelSelection, filters, notes, isBusinessAccount) => {
    const labels = {};
    for (const selectedLabel of labelSelection) {
        // Sanitize labelName to remove invisible Unicode characters (like LRM U+200E)
        const labelName = selectedLabel.name;
        const sanitizedLabelName = labelName.replace(
            /[\u2000-\u200F\u2028-\u202F\u205F\u2060\uFEFF]/g,
            '',
        );
        ...
        let apiContacts = exportContactsFromSelectedLabel(
            labelName,
            filterOnlyUnsaved ? 'unsaved' : 'all',
            '',
        );

        let finalContacts = apiContacts;

        const TEMPORARY_DISABLE_DB_ENRICHMENT = true;
        if (isBusinessAccount && !TEMPORARY_DISABLE_DB_ENRICHMENT) {
            console.log(`WAXP: Business account detected. Enriching label contacts from DB for label "${labelName}".`);
            try {
                const associationJids = await getLabelAssociationsFromDB(labelId);
                const dbContactsRaw = associationJids.map(jid => {
                    const contact = window.Store.Contact.get(jid);
                    if (!contact) return null;
                    // Format this to match the structure of apiContacts
                    return {
                        name: getContactName(contact),
                        phone: contact.id.user,
                        labelName: sanitizedLabelName,
                        lastContacted: getLastContactedDateTime(
                            contact.id._serialized,
                        ),
                        isArchived: isArchived(contact.id),
                    };
                }).filter(Boolean); // Filter out any null contacts

                // Filter DB contacts by 'unsaved' if needed
                let dbContacts = dbContactsRaw;
                if (filterOnlyUnsaved) {
                    dbContacts = dbContactsRaw.filter(c => {
                        const contact = window.Store.Contact.get(c.phone + '@c.us');
                        return contact && !isMyContact(contact);
                    });
                }

                // Merge and deduplicate
                const contactMap = new Map();
                apiContacts.forEach(c => contactMap.set(c.phone, c));
                dbContacts.forEach(c => contactMap.set(c.phone, c)); // DB contacts will overwrite API contacts
                finalContacts = Array.from(contactMap.values());
                
            } catch (error) {
                console.error(`WAXP: Failed to enrich contacts from IndexedDB for label "${labelName}". Falling back to API method.`, error);
                // Fallback is to just use apiContacts, which is already set
            }
        }
    ...
    if (event.data.origin === EVENT_SOURCES.POPUP) {
        ...
        switch (event.data.eventType) {
            ...
            case EVENTS.EXPORT_CONTACTS: {
                ...
                const meContact = window.Store.Contact.getMeContact();
                const isBusinessAccount = meContact && meContact.isBusiness;
                let notes = {};
...
function getFormattedFileName(fileName) {
    fileName = fileName.replace(/[^\d\w\s]/g, '')
        ? fileName.replace(/[^\d\w\s]/g, '')
        : 'WAXP-group-members';
    const d = new Date();
    fileName = `${fileName} ${d.getDate()}-${
        d.getMonth() + 1
    }-${d.getFullYear()}.xlsx`;
    return fileName;
}
